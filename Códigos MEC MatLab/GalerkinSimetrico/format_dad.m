function [NOS_GEO,NOS,ELEM,CDC,normal,conect]=format_dad(PONTOS,SEGMENTOS,MALHA,CCSeg)

% Programa para formata��o dos dados de entrada
%
%   Autor: �der Lima de Albuquerque
...
for i=1:nnos % La�o sobre os pontos fontes
    pontoi=ELEM(i,2); % Ponto final do elemento
    pontof=ELEM(i,3); % Ponto inicial do elemento
...
for i = 1:nnos
    ...
    p2 = ELEM(i,3);
    %
    %	conect(1,j) = element # with node j last
    %	conect(2,j) = element # with node j first
    %   não considera nó duplo
    %   Ex.: conect(1,1) = elemento cujo nó 1 é o último
    %        conect(2,1) = elemento cujo nó 1 é o primeiro
    conect(2,p1) = i;
    conect(1,p2) = i;
end
