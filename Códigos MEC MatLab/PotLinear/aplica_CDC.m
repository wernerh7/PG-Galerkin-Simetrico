...
% a2 = número do elemento com temperatura prescrita ao qual este nó
% pertence.
...
for el=1:n_el % for sobre os elementos para criar T_PR
    for no=1:2 % for sobre os nós do elemento el
        ...
        todos_valores(2*el-2+no)=valorCDC; % armazerna o valor da condição
...
for i=1 : n_temp_pr % for sobre os nós com temperatura conhecida
    ...
    ind_G=2*i_el+i_no_loc-2; % índice da coluna da matriz G que será 
                 %  trocada
    ...
    if(T_PR(i,4)~=0) % Se a temperatura também é conhecida no segundo 
        ...
        ind_G=2*i_el+i_no_loc-2; % índice da coluna G que será atribuído 
                  % zero
